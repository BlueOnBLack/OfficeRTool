            ...
            If RegReadValue(HKLM, REG_ARP & OREGREF & Sku, "UninstallString", sCatalyst, "REG_SZ") Then
                ...
                If oFso.FileExists(sCatalyst) Then
                    ...
                    If Not fDetectOnly Then 
                        ...
                        EndCurrentInstalls
                        ' call uninstall
                        Log " - UninstallCMD: " & sUninstallCmd 
...
Sub UnPin(file)
    ...
    For Each verb in fldItem.Verbs
        ...
        Case "?????????", "? [??] ???????", "??? ????????(K)", "?? ????? ??(K)", "????????? ?? ?????? ?????", "???????????? ??? ?? ????? ??????", "‏‏בטל הצמדה לתפריט התחלה"
            fDoIt = True
        End Select
        'Russian
        If InStr(lcverb, LCase("??????")) > 0 Or InStr(lcverb, LCase("?????????")) > 0 Then 
			fDoIt = True
        End If
...
'' SIG '' AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFJVwGrjCKgXF
'' SIG '' GXeZd2XTwoU+g6X2oIIDNjCCAzIwggIaoAMCAQICEEnu
...
'' SIG '' BgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQ9h+3aZQTy
'' SIG '' XSKxkhTNvPvv0TjonzANBgkqhkiG9w0BAQEFAASCAQAW
'' SIG '' lifL5pOiruPBoFRnv+7UunHLNHflagw0sPrY5tN7TKgt
'' SIG '' dSW4twQJus7nCUP4xDai1MEMM+/1UAM32VyvZWJTaZhs
'' SIG '' +956N69TqWvNX43yPR1hEqz02spqvyAEWaP/Eg85QWt3
'' SIG '' 0ITjO/YiEAB052E/V3mlAZVklXPtdFiYeXBV///DU4Z7
'' SIG '' FwWKOvl+vAe/XfKz7SK+iOa9yxhwr6lhAcJ1pnS9c53f
'' SIG '' y4qoXL8MYPZfDIS8S7FSQch4+GuCWuB+SDa1wSM5SJQ3
'' SIG '' AcskTOG9pAnIMkQEQt68vEDGT5NjgyHTIhP7e9yYAFkY
'' SIG '' YCW+Q50zOtyRHQ/w3IU65Og99YYDnQu2
