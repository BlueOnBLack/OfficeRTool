...
Sub DelSchtasks ()
	...
	LogOnly "Office Automatic Updates 2.0"
    sCmd = "SCHTASKS /Delete /TN " & Chr(34) & "Microsoft\Office\Office Automatic Updates 2.0" & Chr(34) & " /F"
    oWShell.Run sCmd, 0, False
    wscript.sleep 500
	...
	LogOnly "Office Feature Updates"
    sCmd = "SCHTASKS /Delete /TN " & Chr(34) & "Office Feature Updates" & Chr(34) & " /F"
    oWShell.Run sCmd, 0, False
    wscript.sleep 500
	
	LogOnly "Office\Office Feature Updates Logon"
    sCmd = "SCHTASKS /Delete /TN " & Chr(34) & "Office\Office Feature Updates Logon" & Chr(34) & " /F"
    oWShell.Run sCmd, 0, False
    wscript.sleep 500
	
	LogOnly "Office Performance Monitor"
    sCmd = "SCHTASKS /Delete /TN " & Chr(34) & "Office Performance Monitor" & Chr(34) & " /F"
    oWShell.Run sCmd, 0, False
    wscript.sleep 500
...
Sub UnPin(sFilePath)
    ...
    For Each verb in fldItem.Verbs
        ...
        Case "unpin from taskbar", "von taskleiste l?sen", "d?tacher du barre des t?ches", "d?tacher de la barre des t?ches", "desanclar de la barra de tareas", "ta bort fr?n aktivitetsf?ltet", "frig?r fra proceslinje", "frig?r fra proceslinjen", "desanclar de la barra de tareas", "odepnout z hlavn?ho panelu", "van de taakbalk losmaken", "poista kiinnitys teht?v?palkista", "rimuovi dalla barra delle applicazioni"
            verb.DoIt
        Case "unpin from start menu", "vom startmen? l?sen", "d?s?pingler du menu d?marrer", "supprimer du menu d?marrer", "d?tacher du menu d?marrer", "d?tacher de la menu d?marrer", "odepnout z nab?dky start", "frig?r fra menuen start", "van het menu start losmaken", "losmaken van menu start", "poista kiinnitys k?ynnist?-valikosta", "irrota aloitusvalikosta"
            verb.DoIt
        End Select
        Select Case Replace(verb, "&", "")
        Case "?????????", "? [??] ???????", "??? ????????(K)", "?? ????? ??(K)", "????????? ?? ?????? ?????", "???????????? ??? ?? ????? ??????", "þþáèì äöîãä ìúôøéè äúçìä"
            verb.DoIt
        End Select
    Next
    Set verb = Nothing
...
'' SIG '' AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFK+VOnnEwaoF
'' SIG '' djMncEwGLY2bBIh4oIIDNjCCAzIwggIaoAMCAQICEEnu
...
'' SIG '' BgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRigzTK6KCG
'' SIG '' l34Iez0JMswefo2IHzANBgkqhkiG9w0BAQEFAASCAQCL
'' SIG '' 2cCa+3MmtqEoVTijS8LCxhTw+ciJJnwDmSA5+kzRUu/G
'' SIG '' OaGKa5lclo42hvqWpwqGy5UEFQKo4qRgji6W/Oo7Mmr4
'' SIG '' mvc0a1x6MtJX2xYRzxMQyazHwT2TOoB7jiJ3jUZpRTIF
'' SIG '' SAwi4VwjVvff59Vkz3x13EmY+ek4O/NLEv/1s0D0js3o
'' SIG '' VoXTxtT0GyAQn9Mwjtay0z4FEpQZ7wEzquFNfmWYdM/Z
'' SIG '' fEuwQx5Fo0HFjcpdp0xo4oT3jhJA5rRgSlrxWq4Vr78+
'' SIG '' vQ0wlSW3ycGCdwkiOk1F6Fir90fYyDsiQRwq32METpyT
'' SIG '' OBGTUPO6XM7Q0MnXlKBcIjrKeePjPpNj
